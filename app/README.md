...
## Extension pré-roll v1.1.0

Le fichier **file-collecte-preroll-....json** téléchargé depuis le dashboard est une file d'attente : il contient les vidéos à ouvrir, pas les annonceurs. Les champs `brand` et `advertiserCompany` apparaissent uniquement dans le fichier de résultats exporté par l'extension après les tests.

L'extension v1.1.0 :

- crée une fenêtre YouTube dédiée et la réutilise pour toute la file ;
- peut laisser cette fenêtre derrière la fenêtre principale ou l'afficher ;
- force la lecture muette pour réduire les blocages d'autoplay ;
- affiche un diagnostic et un bouton pour ramener la fenêtre de collecte au premier plan ;
- utilise un délai exécuté dans la page YouTube, avec un watchdog Chrome en secours ;
- conserve une observation `Aucune pub` lorsqu'aucun pré-roll n'est détecté.

Après une mise à jour, supprime l'ancienne extension ou clique sur **Actualiser** dans `chrome://extensions`, puis vérifie que la version affichée est **1.1.0**.
