...
const { classifyVideoForSnapshot, classifySnapshot, isSnapshotClassified } = require('./classification-runtime');
...
async function buildSnapshot(channelKey) {
  ...
  return { ok:true, channel: cfg.key, label: cfg.label, handle: cfg.handle, generatedAt: new Date().toISOString(), count: videos.length, videos, source:'youtube-live', classificationReady:true, classifiedAt:new Date().toISOString() };
}
const liveHeaders = {
...
async function buildAnalyticsReport(channelKey, { startDate, endDate, dimensions='', metrics='views,estimatedMinutesWatched,averageViewDuration', maxResults='500' } = {}) {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    data.ftvMeta = { ...analyticsMode, status:r.status, rowCount:Array.isArray(data.rows)?data.rows.length:0, request:{ ids:params.get('ids'), filters:params.get('filters') || '', dimensions:params.get('dimensions') || '', metrics:params.get('metrics') || '', startDate:params.get('startDate') || '', endDate:params.get('endDate') || '' }, generatedAt:new Date().toISOString(), source:'admin-stored-v112' };
  }
  if (!r.ok || data.error) {
...
async function handler(req, res) {
  ...
  try {
    ...
    if (path === 'admin-refresh-data') {
      ...
      return sendJson(res, 200, { ok:results.every(r=>r.ok), version:'v127', generatedAt:new Date().toISOString(), results }, liveHeaders);
    }
    ...
    if (path === 'cache-status') { if (!(await requireUser(req,res))) return; return sendJson(res, 200, { ok:true, strategy:'v112 no cache: admin stored source of truth + client projection only' }, liveHeaders); }
