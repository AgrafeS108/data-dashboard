...
async function buildAnalyticsReport(channelKey, { startDate, endDate, dimensions='', metrics='views,estimatedMinutesWatched,averageViewDuration', maxResults='500' } = {}) {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    data.ftvMeta = { ...analyticsMode, status:r.status, rowCount:Array.isArray(data.rows)?data.rows.length:0, request:{ ids:params.get('ids'), filters:params.get('filters') || '', dimensions:params.get('dimensions') || '', metrics:params.get('metrics') || '', startDate:params.get('startDate') || '', endDate:params.get('endDate') || '' }, generatedAt:new Date().toISOString(), source:'admin-stored-v113' };
  }
  if (!r.ok || data.error) {
...
async function handler(req, res) {
  ...
  try {
    ...
    if (path === 'admin-refresh-data') {
      ...
      return sendJson(res, 200, { ok:results.every(r=>r.ok), version:'v113', generatedAt:new Date().toISOString(), results }, liveHeaders);
    }
    ...
    if (path === 'cache-status') { if (!(await requireUser(req,res))) return; return sendJson(res, 200, { ok:true, strategy:'v113 no cache: admin stored source of truth + client projection only' }, liveHeaders); }
