...
async function applyAnalyticsScope(params, { channelKey } = {}) {
  const ownerId = getYouTubeContentOwnerId();
  const resolved = channelKey ? await resolveAnalyticsChannel(channelKey) : null;
  if (ownerId) {
    const currentIds = params.get('ids') || '';
    ...
    if (resolved?.resolvedChannelId) {
      params.set('filters', joinAnalyticsFilters(`channel==${resolved.resolvedChannelId}`, params.get('filters')));
    }
    return { mode:'contentOwner', ownerId, channelKey: channelKey || '', channelId: resolved?.resolvedChannelId || '', channelTitle: resolved?.resolvedTitle || '' };
  }
  if (resolved?.resolvedChannelId && !params.has('ids')) params.set('ids', `channel==${resolved.resolvedChannelId}`);
  return { mode:'channel', ownerId:'', channelKey: channelKey || '', channelId: resolved?.resolvedChannelId || '', channelTitle: resolved?.resolvedTitle || '' };
}
  ...
  try {
    ...
    if (path === 'youtube-analytics') {
      ...
      for (const [k,v] of Object.entries(paramsIn || {})) {
        if (['channelKey','dashboardChannel','accessToken'].includes(k)) continue;
      ...
      const r = await fetch(`https://youtubeanalytics.googleapis.com/v2/reports?${params.toString()}`, { headers:{ Authorization:`Bearer ${accessToken}` } });
      const data = await r.json().catch(()=>({}));
      if (data && typeof data === 'object' && !Array.isArray(data)) data.ftvMeta = { ...analyticsMode, request:{ ids:params.get('ids'), filters:params.get('filters') || '', dimensions:params.get('dimensions') || '', metrics:params.get('metrics') || '' } };
