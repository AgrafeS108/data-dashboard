...
function profileAllowsChannel(profile, channel) {
  const list = Array.isArray(profile?.allowed_channels) ? profile.allowed_channels.filter(Boolean) : [];
  if (!list.length) return true; // aucune restriction = toutes les chaînes
  return list.includes(String(channel || ''));
}
async function requireChannelAccess(req, res, channel) {
  const current = await requireUser(req, res);
  if (!current) return null;
  if (!profileAllowsChannel(current.profile, channel)) {
    sendJson(res, 403, { ok:false, error:'Chaîne non autorisée pour ce compte.', channel }, {'Cache-Control':'no-store'});
    return null;
  }
  return current;
}
...
async function handler(req, res) {
  ...
  try {
    ...
    if (path === 'auth-login') {
      ...
      profile = await enrichProfile(profile);
      return sendJson(res, 200, { ok:true, user:{ id:user.id, email:user.email }, profile:{ role:profile.role, status:profile.status, full_name:profile.full_name, allowed_channels:profile.allowed_channels || [] } }, {'Cache-Control':'no-store'});
    }
    ...
    if (path === 'auth-me') {
      ...
      return sendJson(res, 200, { ok:true, configured:true, user:{ id:current.user.id, email:current.user.email }, profile:{ role:current.profile.role, status:current.profile.status, full_name:current.profile.full_name, allowed_channels:current.profile.allowed_channels || [] } }, {'Cache-Control':'no-store'});
    }
    ...
    if (path === 'admin-users') {
      ...
      if (req.method === 'GET') {
        ...
        const enriched = await Promise.all((profiles || []).map(p => enrichProfile(p)));
        return sendJson(res, 200, { ok:true, users:enriched }, {'Cache-Control':'no-store'});
      }
    ...
    if (path === 'admin-status') {
      if (!requireAdmin(req,res)) return;
      return sendJson(res, 200, { ok:true, env:{ ADMIN_PASSWORD:!!process.env.ADMIN_PASSWORD, YOUTUBE_API_KEY:!!process.env.YOUTUBE_API_KEY, GOOGLE_CLIENT_ID:!!process.env.GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET:!!process.env.GOOGLE_CLIENT_SECRET, YOUTUBE_REFRESH_TOKEN:!!process.env.YOUTUBE_REFRESH_TOKEN, ANTHROPIC_API_KEY:!!process.env.ANTHROPIC_API_KEY, ALLOWED_ORIGINS:!!process.env.ALLOWED_ORIGINS, SUPABASE_URL:!!process.env.SUPABASE_URL, SUPABASE_ANON_KEY:!!(process.env.SUPABASE_ANON_KEY || process.env.SUPABASE_PUBLISHABLE_KEY), SUPABASE_SERVICE_ROLE_KEY:!!(process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SECRET_KEY) }});
    }
    if (path === 'admin-test') {
      if (!requireAdmin(req,res)) return;
    ...
    if (path === 'snapshot-channel' || path === 'snapshot') {
      ...
      if (!(await requireChannelAccess(req,res,channel))) return;
