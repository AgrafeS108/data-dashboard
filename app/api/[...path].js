...
async function buildSnapshot(channelKey) {
  ...
  return { ok:true, channel: cfg.key, label: cfg.label, handle: cfg.handle, generatedAt: new Date().toISOString(), count: videos.length, videos, source:'youtube-live' };
}
const liveHeaders = {
...
function snapshotChunksTableName() { return 'dashboard_snapshot_chunks'; }
async function deleteSnapshotChunks(channel) {
  return supabaseFetch(`/rest/v1/${snapshotChunksTableName()}?channel=eq.${encodeURIComponent(channel)}`, { method:'DELETE', service:true, headers:{ Prefer:'return=minimal' } });
}
async function insertSnapshotChunks(channel, jsonText) {
  const size = Number(process.env.SNAPSHOT_CHUNK_SIZE || 180000);
  const rows = [];
  for (let i = 0; i < jsonText.length; i += size) rows.push({ channel, idx: rows.length, chunk: jsonText.slice(i, i + size) });
  if (!rows.length) rows.push({ channel, idx: 0, chunk: '{}' });
  for (let i = 0; i < rows.length; i += 8) {
    await supabaseFetch(`/rest/v1/${snapshotChunksTableName()}`, { method:'POST', service:true, body: rows.slice(i, i + 8), headers:{ Prefer:'return=minimal' } });
  }
  return rows.length;
}
async function readSnapshotChunks(channel) {
  const rows = await supabaseFetch(`/rest/v1/${snapshotChunksTableName()}?channel=eq.${encodeURIComponent(channel)}&select=idx,chunk&order=idx.asc`, { service:true });
  if (!Array.isArray(rows) || !rows.length) return null;
  const text = rows.map(r => r.chunk || '').join('');
  try { return JSON.parse(text); } catch(e) { const err = new Error('Snapshot stocké illisible côté Supabase. Relance une actualisation admin.'); err.data = { channel, chunks: rows.length, parseError: e.message }; throw err; }
}
async function upsertDashboardSnapshot(snapshot, meta = {}) {
  ...
  const videoCount = Array.isArray(snapshot.videos) ? snapshot.videos.length : 0;
  const lastVideoPublishedAt = (snapshot.videos || []).map(v => v.publishedAt).filter(Boolean).sort().pop() || null;
  const jsonText = JSON.stringify(snapshot);
  await deleteSnapshotChunks(channel);
  const chunkCount = await insertSnapshotChunks(channel, jsonText);
  const payload = {
    channel,
    payload: { ok:true, channel, label:snapshot.label, generatedAt:snapshot.generatedAt, count:videoCount, videoCount, lastVideoPublishedAt, chunked:true, chunkCount, byteLength:Buffer.byteLength(jsonText,'utf8') },
    video_count: videoCount,
    last_video_published_at: lastVideoPublishedAt,
    source: meta.source || 'admin-refresh-chunked',
...
async function getDashboardSnapshot(channel) {
  ...
  if (!row) return null;
  const fullPayload = await readSnapshotChunks(channel);
  if (fullPayload) row.payload = fullPayload;
  return row;
}
...
async function buildAnalyticsReport(channelKey, { startDate, endDate, dimensions='', metrics='views,estimatedMinutesWatched,averageViewDuration', maxResults='500' } = {}) {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    data.ftvMeta = { ...analyticsMode, status:r.status, rowCount:Array.isArray(data.rows)?data.rows.length:0, request:{ ids:params.get('ids'), filters:params.get('filters') || '', dimensions:params.get('dimensions') || '', metrics:params.get('metrics') || '', startDate:params.get('startDate') || '', endDate:params.get('endDate') || '' }, generatedAt:new Date().toISOString(), source:'admin-stored-v114' };
  }
  if (!r.ok || data.error) {
...
async function handler(req, res) {
  ...
  try {
    ...
    if (path === 'admin-refresh-data') {
      ...
      return sendJson(res, 200, { ok:results.every(r=>r.ok), version:'v114', generatedAt:new Date().toISOString(), results }, liveHeaders);
    }
    ...
    if (path === 'cache-status') { if (!(await requireUser(req,res))) return; return sendJson(res, 200, { ok:true, strategy:'v114 no cache: admin stored source of truth + client projection only' }, liveHeaders); }
    if (path === 'snapshot-channel' || path === 'snapshot') {
      if (!(await requireUser(req,res))) return;
      const channel = String(q.channel || 'sport');
      const stored = await getDashboardSnapshot(channel);
