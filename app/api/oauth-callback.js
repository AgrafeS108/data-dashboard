...
module.exports = async function handler(req, res) {
  ...
  res.statusCode = 200;
  res.end(`<!doctype html><meta charset="utf-8"><title>OAuth OK</title><body style="font-family:Arial;padding:32px;background:#f6f7fb;color:#111827"><h1>Connexion YouTube Analytics réussie</h1><p>Copie le refresh token ci-dessous dans Vercel → Environment Variables → <b>YOUTUBE_REFRESH_TOKEN</b>, puis redéploie.</p><textarea style="width:100%;height:140px;padding:14px;border-radius:12px;border:1px solid #d1d5db">${data.refresh_token || ''}</textarea><pre style="padding:16px;background:white;border-radius:12px;white-space:pre-wrap">${JSON.stringify({scope:data.scope,expires_in:data.expires_in,token_type:data.token_type},null,2)}</pre><p><a href="/admin.html">Retour admin</a></p></body>`);
};
