(() => {
  if (globalThis.__YT_PREROLL_COLLECTOR_ACTIVE__) return;
  globalThis.__YT_PREROLL_COLLECTOR_ACTIVE__ = true;

  const shared = globalThis.YTPreRollShared;
  if (!shared) {
    globalThis.__YT_PREROLL_COLLECTOR_ACTIVE__ = false;
    return;
  }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type === "YT_PREROLL_PING") {
      sendResponse({ ready: true, token, page: location.href });
      return false;
    }
