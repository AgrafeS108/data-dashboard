...
function exportCollectorQueue() {
  ...
  downloadJsonFile(`FILE-VIDEOS-A-TESTER-${sanitizeFilename(state.channel?.title)}-${new Date().toISOString().slice(0, 10)}.json`, {
  ...
  els.collectorMessage.textContent = `${videos.length} vidéo${videos.length > 1 ? "s" : ""} ajoutée${videos.length > 1 ? "s" : ""} à la file JSON. Ce fichier ne contient pas encore d’annonceur : importe-le dans l’extension, lance la collecte, puis réimporte le JSON de résultats.`;
}

function extractCollectorObservations(parsed) {
  ...
  const looksLikeQueue = /queue/i.test(String(parsed.schema || "")) || Array.isArray(parsed.videos);
  if (looksLikeQueue) {
    throw new Error("Tu as sélectionné la FILE DE COLLECTE (les vidéos à tester), pas le fichier de résultats. Importe ce fichier dans l’extension Chrome, clique sur « Démarrer », attends la fin, puis clique dans l’extension sur « Exporter le fichier RÉSULTATS JSON ». Le bon fichier commence par « RESULTATS-PREROLL ».");
  }
  return null;
}
...
async function importCollectorResults(file) {
  ...
  const observations = extractCollectorObservations(parsed);
  if (!Array.isArray(observations)) {
    throw new Error("Ce JSON n’est pas un export de résultats reconnu. Le bon fichier doit venir du bouton « Exporter le fichier RÉSULTATS JSON » de l’extension.");
  }
  if (!observations.length) {
    throw new Error("Le fichier de résultats ne contient encore aucune observation. La collecte n’a pas démarré ou aucune vidéo n’a été finalisée dans l’extension.");
  }
