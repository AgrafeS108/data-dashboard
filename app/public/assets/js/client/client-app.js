...
function App() {
    ...
    const globalSearchVideos = useMemo(() => {
        ...
        if (!q)
            return [];
        return presentationSearchBaseVideos.filter(v => videoMatchesSearch(v, q));
    ...
    const presentationEventOptions = useMemo(() => window.ftvBuildPresentationEvents(allVideos).slice(0, 80), [allVideos, overrideNonce]);
