...
function getSearchIndex(v) {
    ...
    const cls = ftvResolveVideoClassification(v);
...
const CLIENT_CLASSIFICATION_PROFILE = window.FTV_CLASSIFICATION_PROFILE || 'admin-taxonomy-title-first';
window.FTV_CLASSIFICATION_PROFILE = CLIENT_CLASSIFICATION_PROFILE;
function ftvStoredClassification(v) {
    if (!v || typeof v !== 'object' || !v.classification) return null;
    return {
        s: v.classification.sport || v.sport || 'Autres sports',
        c: v.classification.competition || v.competition || 'Non classé',
        i: v.classification.icon || '•',
        bg: v.classification.bg || '#F5F7FA',
        fg: v.classification.fg || '#344054',
        _debug: {
            confidence: v.classification.confidence || 'stored_admin',
            score: Number(v.classification.score || 0),
            source: v.classification.source || 'admin_classification',
            kws: Array.isArray(v.classification.keywords) ? v.classification.keywords : []
        }
    };
}
function ftvStoredClassificationProfile(v) {
    return String(v?.classification?.profile || v?.classificationProfile || v?.classification_profile || '');
}
function ftvClassificationIsWeakFallback(cls) {
    if (!cls || !cls.s) return true;
    const confidence = String(cls._debug?.confidence || cls.confidence || '').toLowerCase();
    const source = String(cls._debug?.source || cls.source || '').toLowerCase();
    const score = Number(cls._debug?.score ?? cls.score ?? 0) || 0;
    if (/fallback|non.?classe|low/.test(confidence + ' ' + source) && score <= 0) return true;
    return /^(autres sports|autres contenus)$/i.test(String(cls.s || '')) && score <= 0;
}
function ftvClassificationDiffers(a, b) {
    if (!a || !b) return !!(a || b);
    return String(a.s || '') !== String(b.s || '') || String(a.c || '') !== String(b.c || '');
}
function ftvApplyResolvedClassification(v, cls) {
    if (!v || typeof v !== 'object' || !cls) return cls;
    ...
    try {
        ...
        v.classification = {
            ...
            sport: cls.s,
            competition: cls.c,
            icon: cls.i || v.classification?.icon || '•',
            bg: cls.bg || v.classification?.bg || '#F5F7FA',
            fg: cls.fg || v.classification?.fg || '#344054',
            confidence: cls._debug?.confidence || cls.confidence || v.classification?.confidence || 'client_resolved',
            score: Number(cls._debug?.score ?? cls.score ?? v.classification?.score ?? 0) || 0,
            source: cls._debug?.source || cls.source || v.classification?.source || 'client_resolved',
            keywords: cls._debug?.kws || cls.keywords || v.classification?.keywords || [],
            profile: CLIENT_CLASSIFICATION_PROFILE
        };
        v.classificationProfile = CLIENT_CLASSIFICATION_PROFILE;
    } catch(e) {}
    return cls;
}
function ftvResolveVideoClassification(v) {
    if (!v || typeof v !== 'object') return classify(v);
    const stored = ftvStoredClassification(v);
    const profileOk = ftvStoredClassificationProfile(v) === CLIENT_CLASSIFICATION_PROFILE;
    const hasManualOverride = !!(v.id && window.FTVLearning?.getOverride && window.FTVLearning.getOverride(v.id));
    if (profileOk && stored && !hasManualOverride) return ftvApplyResolvedClassification(v, stored);
    let current = null;
    try { current = window.classify ? window.classify(v) : null; } catch(e) { current = null; }
    if (hasManualOverride && current && current.s) return ftvApplyResolvedClassification(v, current);
    if (!stored && current && current.s) return ftvApplyResolvedClassification(v, current);
    if (stored && current && current.s) {
        if (ftvClassificationDiffers(stored, current) && !ftvClassificationIsWeakFallback(current)) return ftvApplyResolvedClassification(v, current);
        if (!profileOk && !ftvClassificationIsWeakFallback(current)) return ftvApplyResolvedClassification(v, current);
    }
    if (stored) return ftvApplyResolvedClassification(v, stored);
    if (current && current.s) return ftvApplyResolvedClassification(v, current);
    return ftvApplyResolvedClassification(v, { s: 'Autres sports', c: 'Non classé', i: '•', bg: '#F5F5F5', fg: '#5A5A5A', _debug: { confidence: 'fallback', score: 0, kws: [] } });
}
window.ftvResolveVideoClassification = ftvResolveVideoClassification;
...
function hydrateVideoDerivedFields(v) {
    ...
    try {
        if (v.contentType)
            v.type = v.contentType;
        else if (window.ftvResolveContentType)
            v.type = window.ftvResolveContentType(v);

        ftvResolveVideoClassification(v);
...
function ftvAddVideoToSportsData(data, v) {
    if (!v || typeof v !== 'object')
        return data;
    let cls = ftvResolveVideoClassification(v);
...
function summarizeAnalyticsBySport(videos){
    const map=new Map();(videos||[]).forEach(v=>{const cls=ftvResolveVideoClassification(v);const key=`${cls.s}||${cls.c}`;const cur=map.get(key)||{sport:cls.s,comp:cls.c,i:cls.i,bg:cls.bg,fg:cls.fg,videos:[]};cur.videos.push(v);map.set(key,cur);});
...
function KpiCard({ label, value, sub, accent, delay, format }) {
    const [m, setM] = useState(false);
    useEffect(() => { const t = setTimeout(() => setM(true), delay); return () => clearTimeout(t); }, []);
    return (React.createElement("div", { className: "ftv-kpi-card", style: { background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 11, padding: '8px 12px',
        ...
        React.createElement("div", { className: "ftv-kpi-label", style: { fontSize: 8.5, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 3 } }, label),
        React.createElement("div", { className: "ftv-kpi-value", style: { fontWeight: 850, fontSize: 23, color: 'var(--text)', letterSpacing: '-0.5px', lineHeight: 1 } }, m ? React.createElement(AnimNum, { value: value, format: format || fmt }) : '—'),
        sub && React.createElement("div", { className: "ftv-kpi-sub", style: { fontSize: 8.8, color: 'var(--text3)', marginTop: 2 } }, sub)));
}
// ─── TYPE TAB ─────────────────────────────────────────────────────────────────
...
function TypeTab({ id, label, count, active, onClick }) {
    return (React.createElement("button", { onClick: onClick, style: { display: 'flex', alignItems: 'center', gap: 6,
            ...
            padding: '4px 11px', cursor: 'pointer', fontSize: 11.5, fontWeight: active ? 600 : 400, transition: 'all .14s' } },
        label,
...
function DetailPanel({ sport, data, totalViews, typeFilter, exportMode, selectedComps, onCheckComp, selectedVideos, onCheckVideo, taxonomyMode = 'sport', onReclassify }) {
    ...
    return (React.createElement("div", { className: `ftv-detail-panel ${compactHeader ? 'is-compact' : ''}`, style: { display: 'flex', flexDirection: 'column', height: '100%', animation: 'slideIn .12s ease' } },
        React.createElement("div", { className: "ftv-detail-header", style: { padding: compactHeader ? '5px 16px' : '8px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center',
            ...
            React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 11 } },
                React.createElement("div", { style: { width: compactHeader ? 24 : 31, height: compactHeader ? 24 : 31, borderRadius: compactHeader ? 8 : 10, background: data.bg, display: 'flex', alignItems: 'center',
                        justifyContent: 'center', fontSize: compactHeader ? 12 : 15, border: `1px solid ${data.fg}22`, transition: 'all .16s ease' } }, data.i),
                React.createElement("div", null,
                    React.createElement("div", { style: { fontWeight: 900, fontSize: compactHeader ? 13 : 17, letterSpacing: '0.3px', color: 'var(--text)', textTransform: 'uppercase', transition: 'font-size .16s ease' } }, sport),
                    !compactHeader && React.createElement("div", { style: { fontSize: 9.5, color: 'var(--text3)', marginTop: 0 } },
                        ...
                        " contenu",
                        allVids.length > 1 ? 's' : ''))),
            React.createElement("div", { className: "ftv-detail-mini-stats", style: { display: 'flex', gap: compactHeader ? 9 : 12, flexWrap: 'wrap', justifyContent: 'flex-end', alignItems: 'center', marginLeft: 12 } }, [
                { v: `${nV} / ${nS}`, l: 'vid/shorts' },
                { v: fmt(totalV), l: 'vues' },
            ...
            ].map(({ v, l, c }) => (React.createElement("div", { key: l, style: { textAlign: 'right', minWidth: compactHeader ? 46 : 58 } },
                React.createElement("div", { style: { fontWeight: 850, fontSize: compactHeader ? 12 : 15, color: c || 'var(--text)' } }, v),
                React.createElement("div", { style: { fontSize: compactHeader ? 7.5 : 8.5, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: compactHeader ? '.55px' : '.7px' } }, l)))))),
        React.createElement("div", { className: "ftv-detail-type-pills", style: { display: 'flex', gap: 6, padding: compactHeader ? '3px 16px' : '5px 16px', borderBottom: '1px solid var(--border)', flexShrink: 0,
            ...
            .filter(([n]) => n > 0).map(([n, l, b, c]) => (React.createElement("div", { key: l, style: { display: 'flex', alignItems: 'center', gap: 5, background: b, borderRadius: 7, padding: compactHeader ? '2px 7px' : '3px 8px' } },
            React.createElement("span", { style: { fontSize: compactHeader ? 9 : 10, color: c, fontWeight: 700 } },
                n,
                " ",
                l))))),
        React.createElement("div", { className: "ftv-comp-list-header", style: { display: 'flex', alignItems: 'center', gap: 10, padding: '5px 16px',
                ...
                return React.createElement("div", { key: key, className: "ftv-prevision-card", style: { background: 'var(--bg2)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden', flexShrink: 0, boxShadow: isExp ? '0 4px 20px rgba(0,0,0,0.08)' : 'none' } },
                    React.createElement("div", { className: "ftv-prevision-head", style: { display: 'flex', alignItems: 'center', gap: 12, padding: '13px 18px', cursor: 'pointer' }, onClick: () => setExpanded(isExp ? null : key) },
                        ...
                        React.createElement("div", { style: { minWidth: 0, flex: 1 } },
                            ...
                            React.createElement("div", { style: { fontSize: 9.5, color: 'var(--text3)', marginTop: 0 } },
                                row.sport,
                                " \u00B7 ",
                                row.count,
                                " contenus \u00B7 ",
                        ...
                        React.createElement("div", { style: { textAlign: 'right', minWidth: 90, flexShrink: 0 } },
                            React.createElement("div", { style: { fontSize: compactHeader ? 7.5 : 8.5, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: compactHeader ? '.55px' : '.7px' } }, "Vues totales"),
                            React.createElement("div", { style: { fontWeight: 800, fontSize: 14, color: row.fg, fontVariantNumeric: 'tabular-nums' } }, fmtFull(row.views))),
                        React.createElement("div", { style: { textAlign: 'right', minWidth: 90, flexShrink: 0 } },
                            React.createElement("div", { style: { fontSize: compactHeader ? 7.5 : 8.5, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: compactHeader ? '.55px' : '.7px' } }, "M\u00E9diane"),
...
function App() {
    ...
    return (React.createElement("div", { className: "ftv-app-layout", style: { display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg)' } },
        ...
        React.createElement("div", { className: "ftv-main-shell", style: { flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' } },
            React.createElement("div", { className: "ftv-main-tabs", style: { padding: '0 14px', background: 'var(--bg2)', borderBottom: '1px solid var(--border)',
                ...
                [['chiffres', '📊 Chiffres'], ['graphiques', '📈 Graphiques'], ['previsions', '🔭 Prévisions'], ['alertes', '🚨 Alertes'], ['copilot', '🧠 Copilot IA'], ['analytics', '📡 Analytics']].map(([id, label]) => (React.createElement("button", { key: id, onClick: () => setMainTab(id), style: {
                        padding: '7px 14px', fontSize: 11.5, fontWeight: mainTab === id ? 700 : 500,
                ...
                React.createElement("div", { style: { marginLeft: 'auto', display: 'flex', alignItems: 'center' } },
                    !userMenuOpen && React.createElement("button", { onClick: () => setUserMenuOpen(true), "aria-label": 'Ouvrir le menu profil', style: {
                            width: 30, height: 30, borderRadius: 12, border: '1px solid var(--border2)', background: 'var(--bg4)',
                ...
                userMenuOpen && React.createElement("div", { className: "ftv-user-menu-layer", onClick: () => setUserMenuOpen(false), style: { position: 'fixed', inset: 0, zIndex: 20000, background: 'rgba(15,23,42,.42)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', display: 'flex', justifyContent: 'flex-end', alignItems: 'stretch', isolation: 'isolate' } },
                    React.createElement("aside", { className: "ftv-user-menu-panel", onClick: e => e.stopPropagation(), style: { position: 'relative', top: 'auto', right: 'auto', bottom: 'auto', height: '100dvh', width: 'min(390px,92vw)', zIndex: 1,
                        ...
                        React.createElement("div", { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 } },
                            ...
                            React.createElement("button", { onClick: () => setUserMenuOpen(false), "aria-label": "Fermer le menu", style: { width: 30, height: 30, borderRadius: 12, border: '1px solid var(--border2)', background: 'var(--bg4)', color: 'var(--text)', cursor: 'pointer', fontSize: 18, lineHeight: 1 } }, "\u00D7")),
                        ...
                        React.createElement("div", { style: { marginTop: 4, display: 'grid', gap: 8, paddingBottom: 8 } },
                            ...
                            null)))),
            React.createElement("div", { className: "ftv-kpi-toolbar", style: { padding: '7px 14px', borderBottom: '1px solid var(--border)', display: 'flex',
                    alignItems: 'center', gap: 8, flexShrink: 0, background: 'var(--bg2)' } },
                ...
                React.createElement("div", { style: { display: 'flex', gap: 7, flexShrink: 0, marginLeft: 'auto', alignItems: 'center', position: 'relative' } },
                    React.createElement("button", { onClick: () => setShowCal(c => !c), style: {
                            ...
                            borderRadius: 9, padding: '6px 11px', fontSize: 11.5, fontWeight: 500, cursor: 'pointer',
                    ...
                    canEditRole && React.createElement("button", { onClick: () => { setExportMode(m => { if (m)
                            ...
                            borderRadius: 9, padding: '6px 11px', fontSize: 11.5, fontWeight: 500, cursor: 'pointer', transition: 'all .15s'
                        } },
            ...
            React.createElement("div", { className: "ftv-filter-toolbar", style: { padding: '6px 14px', display: 'flex', gap: 7, flexShrink: 0,
                ...
                React.createElement("div", { style: { marginLeft: 8, flex: '1 1 360px', maxWidth: 760, minWidth: 240, position: 'relative', display: 'flex', alignItems: 'center' } },
                    ...
                    React.createElement("input", { value: searchInput, onChange: e => setSearchInput(e.target.value), placeholder: taxonomyMode === 'general' ? "Rechercher une vidéo, émission, thématique…" : "Rechercher une vidéo, compétition, sportif…", style: { width: '100%', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 20,
                            padding: '5px 34px 5px 30px', fontSize: 11.8, color: 'var(--text)', outline: 'none', height: 28 } }),
