...
function hydrateVideoDerivedFields(v) {
    ...
    try {
        ...
        const storedCls = v.classification ? {
        ...
        if (storedCls) {
            ...
            try { Object.defineProperty(v, '__ftv_cls', { value: storedCls, configurable: true, enumerable: false }); } catch(e) { v.__ftv_cls = storedCls; }
        }
...
function ftvAddVideoToSportsData(data, v) {
    if (!v || typeof v !== 'object')
        return data;
    let cls = v.__ftv_cls || null;
    if (!cls && v.classification) {
        cls = {
            s: v.classification.sport || v.sport || 'Autres sports',
            c: v.classification.competition || v.competition || 'Non classé',
            i: v.classification.icon || '•',
            bg: v.classification.bg || '#F5F7FA',
            fg: v.classification.fg || '#344054',
            _debug: { confidence: v.classification.confidence || 'stored_admin', score: Number(v.classification.score || 0), source: v.classification.source || 'admin_classification', kws: Array.isArray(v.classification.keywords) ? v.classification.keywords : [] }
        };
    }
    if (!cls) {
        try { cls = window.classify ? window.classify(v) : null; }
        catch (e) { cls = null; }
    }
    if (!cls || !cls.s) {
