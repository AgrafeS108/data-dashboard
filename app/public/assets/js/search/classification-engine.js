...
window.classifyType = function(title, durationSecs, isLive) {
  ...
  const hasTitleHashtag = /#[\w\u00C0-\u017F]+/.test(t);
  const hasEmoji = EMOJI_RE.test(t);
  const hasShortWord = /(^|\s)#?shorts?(\s|$)/i.test(t);
  const explicitLongTitle = /resume complet|résumé complet|integralite|intégralité|match complet|course complete|course complète|replay|documentaire|magazine|interview complete|interview complète|conférence de presse|conference de presse/i.test(t);

  // Hashtags / emojis dans le TITRE : cas majoritaire Shorts sur FTV.
  // On ne laisse passer en vidéo que les formats explicitement longs.
  if ((hasTitleHashtag || hasEmoji || hasShortWord) && !explicitLongTitle) return 'short';

  // Une durée très courte est classée short, sauf si le titre indique clairement
  // un format éditorial long/extrait complet.
  if (secs > 0 && secs <= 75 && !explicitLongTitle) return 'short';
  ...
  // Moins de 3 minutes : souvent short, mais pas automatiquement si le titre a
  // une structure de vidéo éditoriale. Entre 1m15 et 3m, on garde l'arbitrage titre.
  if (secs > 0 && secs < 180 && !explicitLongTitle) return 'short';
