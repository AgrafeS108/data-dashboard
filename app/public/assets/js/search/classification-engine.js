...
async function cachedApiJson(url, { force = false } = {}) {
  ...
  let res;
  try {
    res = await fetch(liveUrl, { cache: 'no-store', headers: { 'Cache-Control': 'no-store' }, signal: controller.signal });
  } catch(e) {
    if (e && e.name === 'AbortError') throw new Error('Chargement trop long : la source admin ne répond pas assez vite. Réessaie ou relance le refresh admin.');
    throw e;
  } finally {
