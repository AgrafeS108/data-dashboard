...
-- V113 — Chunked channel snapshots.
-- Objectif : éviter les timeouts Supabase quand une chaîne contient plusieurs milliers de vidéos.
create table if not exists public.dashboard_snapshot_chunks (
  channel text not null,
...
alter table public.dashboard_snapshot_chunks enable row level security;
grant select, insert, update, delete on table public.dashboard_snapshot_chunks to service_role;
drop policy if exists "Service role manages dashboard snapshot chunks" on public.dashboard_snapshot_chunks;
create policy "Service role manages dashboard snapshot chunks" on public.dashboard_snapshot_chunks for all to service_role using (true) with check (true);
create index if not exists dashboard_snapshot_chunks_channel_idx on public.dashboard_snapshot_chunks(channel, idx);
