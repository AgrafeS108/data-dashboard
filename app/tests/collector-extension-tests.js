...
assert.match(workerSource, /sessionMode: "incognito-tabs"/);
assert.match(workerSource, /chrome\.extension\.isAllowedIncognitoAccess/);
assert.match(workerSource, /incognito,/);
assert.match(workerSource, /chrome\.tabs\.create\(\{\s*windowId: collector\.windowId,/s);
assert.match(workerSource, /Chaque vidéo est toujours fermée/);
assert.doesNotMatch(workerSource, /chrome\.tabs\.update\(existingTab\.id,\s*\{\s*url/s);
