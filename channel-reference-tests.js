...
const STORAGE_KEYS = {
  ...
  exportColumns: "yt-channel-viewer-export-columns-v6",
  legacyExportColumns: ["yt-channel-viewer-export-columns-v5", "yt-channel-viewer-export-columns-v4"],
...
const ESSENTIAL_EXPORT_KEYS = [
  ...
  "tags", "description", "adBrandLatest", "adTestCount"
];
...
function normalizeStoredExportKeys(stored) {
  ...
  for (const key of stored) {
    if (key === "parisTime") {
      ...
      migrated.push("adBrandLatest", "adAdvertiserLatest");
    } else {
      migrated.push(key);
    }
  }
  ...
  try {
    ...
    for (const legacyStorageKey of STORAGE_KEYS.legacyExportColumns) {
      ...
      if (legacyKeys.length) {
        localStorage.setItem(STORAGE_KEYS.exportColumns, JSON.stringify(legacyKeys));
...
function uniqueAdValues(videoId, field) {
  return [...new Set(getAdObservations(videoId).map((observation) => String(observation?.[field] || "").trim()).filter(Boolean))];
}
...
function renderAdObservationEditor(video) {
  ...
  const brand = getPrimaryAdBrand(video.id);
...
async function exportExcel() {
  ...
  try {
    ...
    const channelRows = [
      ...
      ["Observations pré-roll", "Données saisies manuellement par test. La marque est prioritaire ; une vidéo peut conserver plusieurs marques, annonceurs, lieux et formats observés."],
