...
window.classify = function(input, tags=[], description='') {
  ...
  // 1. Override manuel : priorité absolue. Si l'utilisateur a explicitement
  //    corrigé cette vidéo, on retourne sa correction sans rien recalculer.
  const videoId = input && typeof input === 'object' ? (input.id || input.videoId) : null;
  if (videoId) {
    const ov = window.FTVLearning.getOverride(videoId);
    if (ov && ov.sport) {
      // On reconstruit un retour de la même forme que classify normal.
      // On cherche le sport dans SR_SCORED pour récupérer ses couleurs/icône.
  ...
  // 2. Cache standard.
  if (canCache && __FTV_CLASSIFY_CACHE.has(input)) return __FTV_CLASSIFY_CACHE.get(input);

  // 3. Classification normale.
  const result = __FTV_CLASSIFY_RAW(input, tags, description);
...
window.classifyType = function(title, durationSecs, isLive) {
  ...
  const t = (title||'').trim();
  if (/#[\w\u00C0-\u017F]+/.test(t)) return 'short';
  if (EMOJI_RE.test(t)) return 'short';
  // Pattern « Compétition : Titre » avec séparateur explicite. On accepte
  // : | — – ainsi qu'un tiret simple entouré d'espaces (« Top 14 - J22 ... »).
  // On exige au moins 3 caractères avant le séparateur pour éviter les
  // acronymes ponctués (« A : B »).
  if (/^[^:|—–]{3,}\s+[:|—–]\s*\S/.test(t)) return 'video';
  if (/^[^:|—–\-]{3,}\s+-\s+\S/.test(t)) return 'video';
  if (durationSecs && durationSecs > 180) return 'video';
  return 'short';
};
...
window.buildSports = function(videos) {
  ...
  videos.forEach(v => {
    ...
    if (!data[cls.s]) data[cls.s] = {i:cls.i, bg:cls.bg, fg:cls.fg, comps:{}, total:0, views:0};
    if (!data[cls.s].comps[cls.c]) data[cls.s].comps[cls.c] = {videos:[], views:0};
...
window.CACHE_SCHEMA_VERSION = 'v88';
...
window.saveCache = function(videos, channelKey=window.DEFAULT_CHANNEL_KEY) {
  const key = window.getCacheKey(channelKey);
  const attempts = (window.isHeavyChannel ? window.isHeavyChannel(channelKey) : channelKey === 'francetv')
    ? [{desc:700,tags:15},{desc:250,tags:8},{desc:0,tags:0}]
    : [{desc:2200,tags:40},{desc:1400,tags:30},{desc:700,tags:15},{desc:250,tags:8}];
  for(const a of attempts) {
    try {
      const payload = {ts:Date.now(), schema:window.CACHE_SCHEMA_VERSION, channelKey, videos:slimVideosForCache(videos, a.desc, a.tags)};
      localStorage.setItem(key, JSON.stringify(payload));
      return true;
    } catch(e) {}
  }
  try { console.warn('Cache non sauvegardé : quota localStorage dépassé pour', channelKey); } catch(e) {}
  return false;
};
window.loadCache = function(channelKey=window.DEFAULT_CHANNEL_KEY) {
  const keys = window.getCacheKeys ? window.getCacheKeys(channelKey) : [window.getCacheKey(channelKey)];
  for (const key of keys) {
    try {
      const raw = localStorage.getItem(key);
      if (!raw) continue;
      // Protection anti-page-blanche : les anciens caches FRANCE TV pouvaient
      // devenir trop gros pour être parsés rapidement. On les ignore proprement.
      if ((window.isHeavyChannel ? window.isHeavyChannel(channelKey) : channelKey === 'francetv') && raw.length > window.CACHE_MAX_RAW_BYTES) {
        try { localStorage.removeItem(key); } catch(e) {}
        continue;
      }
      const obj = JSON.parse(raw);
      if (!obj.videos || !obj.ts || !obj.videos.length) continue;
      if (obj.channelKey && obj.channelKey !== channelKey) { try { localStorage.removeItem(key); } catch(e) {} continue; }
      if (window.ftvCacheBelongsToChannel && !window.ftvCacheBelongsToChannel(obj, channelKey)) { try { localStorage.removeItem(key); } catch(e) {} continue; }
      // Ré-applique uniquement le type short/video/live. La classification complète
      // reste en cache mémoire par objet vidéo et n'est pas recalculée à chaque navigation.
      obj.videos = obj.videos.map(v => {
        if (v.type === 'live') return v;
        const dur = window.getDurationSecs(v.duration);
        return {...v, channelKey: v.channelKey || channelKey, type: window.classifyType(v.title, dur, false)};
      });
      // Migration douce : si on a lu un ancien cache, on réécrit dans la clé v28.
      if (key !== window.getCacheKey(channelKey)) window.saveCache(obj.videos, channelKey);
      return obj;
    } catch(e) {}
  }
  return null;
...
const YT_HTTP_CACHE_TTL_MS = 6 * 60 * 60 * 1000; // 6h : reload quasi instantané

async function cachedApiJson(url, { force = false } = {}) {
  const fullUrl = new URL(url, window.location.origin).toString();
  const metaKey = 'ftv_http_cache_meta_' + fullUrl;

  if ('caches' in window && !force) {
    try {
      const ts = Number(localStorage.getItem(metaKey) || 0);
      const cache = await caches.open(YT_HTTP_CACHE_NAME);
      const cached = await cache.match(fullUrl);
      if (cached && ts && (Date.now() - ts) < YT_HTTP_CACHE_TTL_MS) {
        return await cached.clone().json();
      }
    } catch (e) {}
  }

  const res = await fetch(fullUrl, { cache: force ? 'reload' : 'default' });
  ...
  if ('caches' in window) {
    try {
      const cache = await caches.open(YT_HTTP_CACHE_NAME);
      await cache.put(fullUrl, new Response(JSON.stringify(data), {
        status: 200,
        headers: { 'Content-Type': 'application/json', 'X-FTV-Cache-Ts': String(Date.now()) }
      }));
...
window.fetchChannelSnapshot = async function(channelKey, { force = false } = {}) {
  ...
  const url = `/api/snapshot-channel?channel=${encodeURIComponent(requested)}&schema=${encodeURIComponent(window.CACHE_SCHEMA_VERSION || 'v88')}${force ? '&force=1' : ''}`;
...
window.refreshPublicStatsLive = async function(videos, onProgress) {
  const arr = Array.isArray(videos) ? videos : [];
  const ids = arr.map(v => v.id).filter(Boolean);
  if (!ids.length) return arr;
  const statsById = {};
  for (let i = 0; i < ids.length; i += 50) {
    const chunk = ids.slice(i, i + 50);
    if (onProgress) onProgress(`Actualisation live des vues publiques ${Math.min(i + 50, ids.length)}/${ids.length}…`);
...
;
document.documentElement.classList.add('ftv-auth-checking');
;
...
(function(){
  ...
  function normalizeProfile(payload){
    ...
    return { email:user.email||p.email||'', firstName:n.firstName, lastName:n.lastName, fullName:p.full_name||user.email||'', role:p.role||'viewer', status:p.status||'active', licenseActive:p.status!=='disabled' };
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){ $('ftvAuthForm')?.addEventListener('submit',login); $('ftvAuthSubmit')?.addEventListener('click',login); me(); });
})();
...
function PredictiveCopilotView({ allVideos, currentVideos, sportsData, totalViews, channelKey, dateRange, analyticsBundle }) {
    ...
    const watchlist = React.useMemo(()=>ftvCopilotBuildWatchlist(events, channelKey), [events, channelKey, watchTick]);
        ...
        React.createElement('div', { className:'ftv-copilot-grid', style:{gridTemplateColumns:'minmax(0,1.08fr) minmax(360px,.92fr)'} },
            React.createElement('div', { className:'ftv-copilot-panel' },
                ...
                selected ? React.createElement('div', null,
                    ...
                    React.createElement('button',{onClick:()=>{ftvCopilotToggleWatch(selected.key, channelKey); setWatchTick(x=>x+1);},style:{height:38,borderRadius:12,border:'1px solid var(--border2)',background:'var(--bg2)',fontWeight:900,fontFamily:'var(--font)',padding:'0 14px',cursor:'pointer'}}, watchlist.find(w=>w.key===selected.key)?'Retirer de la watchlist':'Ajouter à la watchlist')
...
function App() {
    ...
    const handleChannelChange = useCallback((nextKey) => {
        ...
        const run = () => {
            ...
            if (!state) {
                let channelCache = loadCache(cfg.key);
                if (channelCache && window.ftvCacheBelongsToChannel && !window.ftvCacheBelongsToChannel(channelCache, cfg.key)) channelCache = null;
    ...
    const handleLoad = useCallback(async () => {
        ...
        try {
            ...
            try {
                ...
                videos = prepareVideosForDashboard((snapshot.videos || []).map(v => ({...v, channelKey})));
            } catch (snapshotError) {
