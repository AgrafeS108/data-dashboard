...
function hydrateVideoDerivedFields(v) {
    if (!v || typeof v !== 'object')
        return v;
    try {
        if (!Object.prototype.hasOwnProperty.call(v, '__ftv_cls')) {
            Object.defineProperty(v, '__ftv_cls', { value: classify(v), configurable: true, enumerable: false });
        }
        if (!Object.prototype.hasOwnProperty.call(v, '__ftv_search')) {
            Object.defineProperty(v, '__ftv_search', { value: getSearchIndex(v), configurable: true, enumerable: false });
        }
...
function prepareVideosForDashboard(videos, maxSync = 30) {
...
async function queryYouTubeAnalytics(accessToken,params){
    const res=await fetch('/api/youtube-analytics',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(params||{}),signal:AbortSignal.timeout(60000)});
    const data=await res.json().catch(()=>({}));
    if(!res.ok||data.error)throw new Error(data?.error?.message||data?.message||`Erreur YouTube Analytics ${res.status}`);
    return data;
}
...
function summarizeAnalyticsForVideos(videos){
    const rows=videos||[];const views=sumAnalyticsMetric(rows,'analyticsViews','views');const watch=sumAnalyticsMetric(rows,'watchTimeMinutes');const impressions=sumAnalyticsMetric(rows,'impressions');
...
async function fetchAnalyticsBundle({accessToken,videos,dateRange,channelKey,onProgress}){
    if(!accessToken)throw new Error('Connexion YouTube Analytics requise.');
    ...
    const videoIds=[...new Set(sourceVideos.map(v=>v.id).filter(Boolean))];const errors=[];let done=0;
    for(const idsChunk of chunkArray(videoIds,35)){
        const filters=`video==${idsChunk.join(',')}`;
        try{const data=await queryYouTubeAnalytics(accessToken,{ids,startDate,endDate,metrics:'views,estimatedMinutesWatched,averageViewDuration,averageViewPercentage,likes,comments,shares,subscribersGained,subscribersLost',dimensions:'video',filters,maxResults:idsChunk.length,sort:'-views'});analyticsRowsToObjects(data).forEach(row=>mergeAnalyticsRow(byId.get(row.video),row));}
        catch(e){try{const data=await queryYouTubeAnalytics(accessToken,{ids,startDate,endDate,metrics:'views,estimatedMinutesWatched,averageViewDuration,likes,comments',dimensions:'video',filters,maxResults:idsChunk.length,sort:'-views'});analyticsRowsToObjects(data).forEach(row=>mergeAnalyticsRow(byId.get(row.video),row));}catch(e2){errors.push(e2.message||String(e2));}}
        try{const data2=await queryYouTubeAnalytics(accessToken,{ids,startDate,endDate,metrics:'impressions,impressionClickThroughRate',dimensions:'video',filters,maxResults:idsChunk.length,sort:'-impressions'});analyticsRowsToObjects(data2).forEach(row=>mergeAnalyticsRow(byId.get(row.video),row));}catch(e){errors.push(`Impressions/CTR: ${e.message||e}`);}
        done+=idsChunk.length;if(onProgress)onProgress(`Analytics vidéos : ${done}/${videoIds.length}`);await new Promise(r=>setTimeout(r,0));
    }
    const safeQuery=async(name,config)=>{try{return{name,data:await queryYouTubeAnalytics(accessToken,{ids,startDate,endDate,...config})};}catch(e){errors.push(`${name}: ${e.message||e}`);return{name,data:null};}};
    const [daily,traffic,devices,countries]=await Promise.all([
        safeQuery('daily',{metrics:'views,estimatedMinutesWatched,averageViewDuration,likes,comments,subscribersGained',dimensions:'day',sort:'day',maxResults:500}),
        safeQuery('traffic',{metrics:'views,estimatedMinutesWatched,averageViewDuration',dimensions:'insightTrafficSourceType',sort:'-views',maxResults:50}),
        safeQuery('devices',{metrics:'views,estimatedMinutesWatched,averageViewDuration',dimensions:'deviceType',sort:'-views',maxResults:50}),
    ...
    return{channelKey,startDate,endDate,loadedAt:Date.now(),videos:sourceVideos,byVideo:Object.fromEntries(sourceVideos.map(v=>[v.id,v.analytics||{}])),summary,daily:daily.data,traffic:traffic.data,devices:devices.data,countries:countries.data,errors:[...new Set(errors)].slice(0,8)};
}
...
function getExportRowsFromSelection({ selectedSports, selectedComps, selectedVideos, sportsData, allVideos }) {
    ...
    const addVideo = (v, sportName, compName, source) => {
        ...
        const baseRow = {
            ...
            duration: getDurationSecs(v.duration || v.dur || v.contentDetails?.duration || '') || '', views: Number(v.views || 0),
...
function KpiCard({ label, value, sub, accent, delay, format }) {
    const [m, setM] = useState(false);
    useEffect(() => { const t = setTimeout(() => setM(true), delay); return () => clearTimeout(t); }, []);
    return (React.createElement("div", { style: { background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: '13px 16px',
        ...
        accent && React.createElement("div", { style: { position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: accent, borderRadius: '12px 12px 0 0' } }),
        React.createElement("div", { style: { fontSize: 9.5, fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '1.1px', marginBottom: 5 } }, label),
        React.createElement("div", { style: { fontWeight: 800, fontSize: 28, color: 'var(--text)', letterSpacing: '-0.5px', lineHeight: 1 } }, m ? React.createElement(AnimNum, { value: value, format: format || fmt }) : '—'),
        sub && React.createElement("div", { style: { fontSize: 10, color: 'var(--text3)', marginTop: 3 } }, sub)));
}
// ─── TYPE TAB ─────────────────────────────────────────────────────────────────
...
function VideoRow({ v, exportMode, checked, onCheck, fg, onReclassify }) {
    ...
    return (React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 8, padding: '7px 20px 7px 36px',
        ...
        React.createElement("span", { style: { flex: 1, fontSize: 12, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, title: tooltip }, v.title),
...
function CompRow({ name, compData, maxViews, fg, bg, index, exportMode, checked, onCheck, selectedVideos, onCheckVideo, onReclassify }) {
    ...
    return (React.createElement("div", { style: { borderBottom: '1px solid var(--border)' } },
        React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 10, padding: '11px 18px', cursor: 'pointer', transition: 'background .14s' }, onClick: () => setExpanded(e => !e), onMouseEnter: e => e.currentTarget.style.background = 'rgba(0,0,0,0.018)', onMouseLeave: e => e.currentTarget.style.background = 'transparent' },
...
function DetailPanel({ sport, data, totalViews, typeFilter, exportMode, selectedComps, onCheckComp, selectedVideos, onCheckVideo, taxonomyMode = 'sport', onReclassify }) {
    ...
    const filteredComps = useMemo(() => (Object.entries(data.comps).map(([n, cd]) => {
        ...
        if (!vids.length)
            return null;
        return [n, { videos: vids, views: vids.reduce((s, v) => s + v.views, 0) }];
    ...
    return (React.createElement("div", { style: { display: 'flex', flexDirection: 'column', height: '100%', animation: 'slideIn .12s ease' } },
        React.createElement("div", { style: { padding: '14px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center',
                justifyContent: 'space-between', flexShrink: 0, background: 'var(--bg2)' } },
            React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 11 } },
                React.createElement("div", { style: { width: 40, height: 40, borderRadius: 11, background: data.bg, display: 'flex', alignItems: 'center',
                        justifyContent: 'center', fontSize: 19, border: `1px solid ${data.fg}22` } }, data.i),
                React.createElement("div", null,
                    React.createElement("div", { style: { fontWeight: 900, fontSize: 20, letterSpacing: '0.3px', color: 'var(--text)', textTransform: 'uppercase' } }, sport),
        ...
        React.createElement("div", { style: { display: 'flex', gap: 7, padding: '8px 18px', borderBottom: '1px solid var(--border)', flexShrink: 0,
                flexWrap: 'wrap', background: 'var(--bg3)' } }, [[nV, 'Vidéos', TYPE_BG.video, TYPE_COLORS.video], [nS, 'Shorts', TYPE_BG.short, TYPE_COLORS.short], [nL, 'Lives', TYPE_BG.live, TYPE_COLORS.live]]
            ...
            React.createElement("span", { style: { fontSize: 10.5, color: c, fontWeight: 600 } },
                ...
                " ",
                l))))),
        React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 10, padding: '7px 18px',
            ...
            React.createElement("div", { style: { width: 200, flexShrink: 0, fontSize: 9.5, fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.8px', textAlign: 'right', paddingRight: 0 } }, "Vues totales"),
...
function ChartsView({ sportsData, totalViews, typeFilter, onSelectSport }) {
    ...
    return (React.createElement("div", { style: { height: '100%', overflowY: 'auto', padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 6 } },
        ...
        sorted.map(([sport, data]) => {
            ...
            return (React.createElement("div", { key: sport, style: { background: 'var(--bg2)', borderRadius: 12, border: '1px solid var(--border)',
                    overflow: 'hidden', transition: 'box-shadow .15s', flexShrink: 0,
                    boxShadow: isExp ? '0 4px 16px rgba(0,0,0,0.08)' : 'none' } },
                React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', cursor: 'pointer' }, onClick: () => setExpanded(isExp ? null : sport) },
...
function AnalyticsView({ allVideos, analyticsBundle, analyticsStatus, onConnect, onLoad, analyticsClientId, setAnalyticsClientId, analyticsAccessToken }) {
    ...
    const Table = ({ title, data }) => {
        ...
        return React.createElement("div", { style: { background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' } },
            ...
            React.createElement("div", { style: { maxHeight: 260, overflow: 'auto' } }, objects.length ? React.createElement("table", { style: { width: '100%', borderCollapse: 'collapse', fontSize: 11 } },
                ...
                React.createElement("tbody", null, objects.map((r,i) => React.createElement("tr", { key: i }, cols.map(c => React.createElement("td", { key: c, style: { padding: '7px 10px', borderBottom: '1px solid rgba(0,0,0,.04)', color: 'var(--text2)' } }, typeof r[c] === 'number' ? fmtFull(r[c]) : String(r[c] ?? ''))))))) : React.createElement("div", { style: { padding: 14, color: 'var(--text3)', fontSize: 12 } }, hasAnalytics ? 'Aucune donnée retournée.' : 'Charge Analytics pour afficher ce rapport.')));
    };
    return React.createElement("div", { style: { height: '100%', overflowY: 'auto', padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 14 } },
        React.createElement("div", { style: { background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, display: 'grid', gridTemplateColumns: 'minmax(280px,1fr) 150px 150px', gap: 12, alignItems: 'end' } },
            React.createElement("div", null,
                React.createElement("div", { style: { fontSize: 16, fontWeight: 900, color: 'var(--text)', marginBottom: 4 } }, "YouTube Analytics"),
                React.createElement("div", { style: { fontSize: 11, color: 'var(--text3)', lineHeight: 1.5, marginBottom: 8 } }, "Analytics est géré côté admin : aucune clé ni connexion Google n’est demandée au client. Les résultats sont croisés avec les sports/événements déjà classés."),
                React.createElement("div", { style: { width: '100%', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 9, padding: '8px 10px', fontSize: 12, color: 'var(--text2)' } }, "Configuration sécurisée dans /admin")),
            React.createElement("button", { onClick: () => window.open('/admin','_blank'), style: { height: 38, background: '#0B3F7A', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font)' } }, 'Console admin'),
            React.createElement("button", { onClick: onLoad, style: { height: 38, background: 'var(--red)', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--font)' } }, 'Charger Analytics')),
        React.createElement("div", { style: { fontSize: 12, color: analyticsStatus?.type === 'error' ? '#C0392B' : analyticsStatus?.type === 'ok' ? '#166534' : 'var(--text3)', background: analyticsStatus?.type === 'error' ? '#FEE2E2' : analyticsStatus?.type === 'ok' ? '#DCFCE7' : 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '9px 12px' } }, analyticsStatus?.message || 'Analytics non chargé.'),
        ...
        React.createElement("div", { style: { background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' } },
            ...
            React.createElement("div", { style: { maxHeight: 420, overflow: 'auto' } }, React.createElement("table", { style: { width: '100%', borderCollapse: 'collapse', fontSize: 11 } },
                ...
                React.createElement("tbody", null, (topRows.length ? topRows : rows.slice(0,30)).map(r => React.createElement("tr", { key: `${r.sport}::${r.comp}` },
                    ...
                    React.createElement("td", { style: { padding: '8px 10px', textAlign: 'right' } }, fmtFull(r.videos.length)),
    ...
    return (React.createElement("div", { style: { height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)' } },
        ...
        React.createElement("div", { style: { flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 10 } },
            shownRows.map(row => {
                ...
                return React.createElement("div", { key: key, style: { background: 'var(--bg2)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden', flexShrink: 0, boxShadow: isExp ? '0 4px 20px rgba(0,0,0,0.08)' : 'none' } },
                    React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 12, padding: '13px 18px', cursor: 'pointer' }, onClick: () => setExpanded(isExp ? null : key) },
...
function PredictiveCopilotView({ allVideos, sportsData, totalViews, channelKey, dateRange, analyticsBundle }) {
    const [selectedVideoId, setSelectedVideoId] = React.useState('');
    const videos = allVideos || [];
    const summary = React.useMemo(() => window.PredictionEngine ? window.PredictionEngine.summarize(videos) : {opportunities:[],risks:[],averagePotential:0,count:0}, [videos]);
    const selectedVideo = React.useMemo(() => videos.find(v => v.id === selectedVideoId) || summary.opportunities?.[0]?.video || videos[0], [videos, selectedVideoId, summary]);
    const selectedPrediction = React.useMemo(() => selectedVideo && window.PredictionEngine ? window.PredictionEngine.predictVideo(selectedVideo, videos) : null, [selectedVideo, videos]);
    const compact = window.AIMediaCopilot?.formatCompact || ((n)=>String(Math.round(Number(n)||0)));
    const brief = React.useMemo(() => window.AIMediaCopilot ? window.AIMediaCopilot.buildBrief(summary) : '', [summary]);
    const sportRows = React.useMemo(() => Object.entries(sportsData || {}).map(([name,d]) => ({name, views:Number(d.views||0), videos:Object.values(d.comps||{}).flatMap(c=>c.videos||[])})).sort((a,b)=>b.views-a.views).slice(0,8), [sportsData]);
    const confidenceLabel = selectedPrediction?.confidence === 'high' ? 'confiance forte' : selectedPrediction?.confidence === 'medium' ? 'confiance moyenne' : 'confiance prudente';
    return React.createElement('div', { className:'ftv-copilot-shell' },
        React.createElement('div', { className:'ftv-copilot-hero' },
            React.createElement('div', null,
                React.createElement('div', { style:{display:'flex',gap:8,flexWrap:'wrap',marginBottom:14} },
                    React.createElement('span', { className:'ftv-copilot-pill', style:{background:'rgba(227,6,19,.18)',color:'#fff'} }, 'AI Predictive Media Copilot'),
                    React.createElement('span', { className:'ftv-copilot-pill', style:{background:'rgba(255,255,255,.12)',color:'#e5e7eb'} }, (channelKey||'dashboard').toUpperCase()),
                    React.createElement('span', { className:'ftv-copilot-pill', style:{background:'rgba(255,255,255,.12)',color:'#e5e7eb'} }, analyticsBundle?.loadedAt ? 'Analytics enrichi' : 'Data API + synthétique')
                ),
                React.createElement('h2', { style:{fontSize:38,lineHeight:1,letterSpacing:'-.06em',margin:'0 0 10px',fontWeight:950} }, 'AI Media Copilot'),
                React.createElement('p', { style:{margin:0,color:'rgba(255,255,255,.72)',fontSize:14,lineHeight:1.55,maxWidth:720} }, 'Projections, potentiel de viralité, scénarios synthétiques et recommandations média. Les scores sont des estimations probabilistes basées sur l’historique réel, les vidéos similaires, la vélocité et l’engagement public.'),
                React.createElement('div', { className:'ftv-copilot-kpis' },
                    [['Potentiel moyen', summary.averagePotential + '/100'], ['Vidéos analysées', compact(summary.count)], ['Vues période', compact(totalViews)], ['Opportunités', String(summary.opportunities?.length||0)]].map(([k,v]) => React.createElement('div',{className:'ftv-copilot-card',key:k}, React.createElement('div',{style:{fontSize:10,textTransform:'uppercase',letterSpacing:1.6,color:'rgba(255,255,255,.52)',fontWeight:900}},k), React.createElement('div',{style:{fontSize:28,fontWeight:950,marginTop:6}},v)))
                )
            ),
            React.createElement('div', { className:'ftv-copilot-card', style:{background:'rgba(255,255,255,.08)'} },
                React.createElement('div', { style:{fontSize:12,textTransform:'uppercase',letterSpacing:1.4,color:'rgba(255,255,255,.55)',fontWeight:900,marginBottom:8} }, 'Lecture stratégique'),
                React.createElement('pre', { style:{whiteSpace:'pre-wrap',fontFamily:'var(--font)',fontSize:12,lineHeight:1.55,color:'rgba(255,255,255,.82)',margin:0} }, brief || 'Chargement des signaux…')
            )
        ),
        React.createElement('div', { className:'ftv-copilot-grid' },
            React.createElement('div', { className:'ftv-copilot-panel' },
                React.createElement('div', { style:{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',marginBottom:12,flexWrap:'wrap'} },
                    React.createElement('div', null,
                        React.createElement('h3', { style:{margin:'0 0 4px',fontSize:20,fontWeight:950,letterSpacing:'-.04em'} }, 'Opportunités prioritaires'),
                        React.createElement('div', { style:{fontSize:12,color:'var(--text3)'} }, 'Vidéos avec potentiel de croissance organique ou surperformance probable.')
                    ),
                    React.createElement('select', { value:selectedVideo?.id||'', onChange:e=>setSelectedVideoId(e.target.value), style:{height:38,border:'1px solid var(--border2)',borderRadius:12,padding:'0 10px',fontFamily:'var(--font)',maxWidth:280} },
                        videos.slice().sort((a,b)=>Number(b.views||0)-Number(a.views||0)).slice(0,80).map(v=>React.createElement('option',{key:v.id,value:v.id}, (v.title||'Sans titre').slice(0,72)))
                    )
                ),
                (summary.opportunities||[]).slice(0,8).map(({video,prediction},i)=>React.createElement('div',{className:'ftv-copilot-row',key:video.id||i,onClick:()=>setSelectedVideoId(video.id),style:{cursor:'pointer'}},
                    React.createElement('div',{style:{width:44,height:44,borderRadius:14,background:'linear-gradient(135deg,#EFF6FF,#FEE2E2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20}}, video.i || '▶'),
                    React.createElement('div',{style:{minWidth:0}},
                        React.createElement('div',{style:{fontSize:14,fontWeight:900,color:'var(--text)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}, video.title||'Sans titre'),
                        React.createElement('div',{style:{fontSize:11,color:'var(--text3)',marginTop:3}}, `${compact(video.views)} vues · projection J+7 ${compact(prediction.projected7d[0])} – ${compact(prediction.projected7d[1])}`)
                    ),
                    React.createElement('div',{className:'ftv-copilot-score'}, prediction.potentialScore)
                )),
                !(summary.opportunities||[]).length && React.createElement('div',{style:{padding:20,color:'var(--text3)'}},'Aucune opportunité forte détectée sur cette période.')
            ),
            React.createElement('div', { className:'ftv-copilot-panel' },
                React.createElement('h3', { style:{margin:'0 0 8px',fontSize:20,fontWeight:950,letterSpacing:'-.04em'} }, 'Simulation sélectionnée'),
                selectedVideo && selectedPrediction ? React.createElement(React.Fragment,null,
                    React.createElement('div',{style:{fontSize:13,fontWeight:900,color:'var(--text)',lineHeight:1.25,marginBottom:10}}, selectedVideo.title),
                    React.createElement('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}},
                        [['Viralité', selectedPrediction.viralityProbability + '%'], ['Confiance', confidenceLabel], ['J+24h', `${compact(selectedPrediction.projected24h[0])}–${compact(selectedPrediction.projected24h[1])}`], ['J+30', `${compact(selectedPrediction.projected30d[0])}–${compact(selectedPrediction.projected30d[1])}`]].map(([k,v])=>React.createElement('div',{key:k,style:{background:'#f9fafb',border:'1px solid rgba(17,24,39,.07)',borderRadius:14,padding:10}},React.createElement('div',{style:{fontSize:10,textTransform:'uppercase',letterSpacing:1.2,color:'var(--text3)',fontWeight:900}},k),React.createElement('div',{style:{fontSize:17,fontWeight:950,marginTop:3}},v)))
                    ),
                    React.createElement('div',{style:{fontSize:12,fontWeight:900,margin:'10px 0 8px'}},'Scénarios synthétiques'),
                    (selectedPrediction.scenarios||[]).map((sc,i)=>React.createElement('div',{className:'ftv-copilot-scenario',key:i},
                        React.createElement('div',{style:{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center'}},React.createElement('div',{style:{fontWeight:900,fontSize:13}},sc.label),React.createElement('div',{style:{fontWeight:950,color:'#E30613'}},sc.display)),
                        React.createElement('div',{style:{fontSize:11,color:'var(--text3)',lineHeight:1.45,marginTop:5}},`${sc.rationale} · probabilité ${sc.probability}% · risque ${sc.risk}`)
                    ))
                ) : React.createElement('div',{style:{color:'var(--text3)',padding:12}},'Sélectionne une vidéo.')
            )
        ),
        React.createElement('div', { className:'ftv-copilot-grid' },
            React.createElement('div', { className:'ftv-copilot-panel' },
                React.createElement('h3', { style:{margin:'0 0 10px',fontSize:18,fontWeight:950} }, 'Momentum par catégorie'),
                sportRows.map(row=>React.createElement('div',{key:row.name,style:{display:'grid',gridTemplateColumns:'1fr auto',gap:12,alignItems:'center',padding:'9px 0',borderBottom:'1px solid rgba(17,24,39,.06)'}},
                    React.createElement('div',null,React.createElement('div',{style:{fontWeight:900}},row.name),React.createElement('div',{style:{height:6,borderRadius:99,background:'#eef2f7',marginTop:6,overflow:'hidden'}},React.createElement('div',{style:{height:'100%',width:`${Math.min(100,(row.views/(sportRows[0]?.views||1))*100)}%`,background:'linear-gradient(90deg,#E30613,#143F7D)',borderRadius:99}}))),
                    React.createElement('div',{style:{fontWeight:950,color:'#143F7D'}},compact(row.views))
                ))
            ),
            React.createElement('div', { className:'ftv-copilot-panel' },
                React.createElement('h3', { style:{margin:'0 0 10px',fontSize:18,fontWeight:950} }, 'Future-ready Analytics'),
                ['CTR', 'Watch time', 'Rétention', 'Impressions', 'Sources de trafic', 'Abonnés vs non abonnés', 'Géographie'].map(x=>React.createElement('div',{key:x,style:{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 0',borderBottom:'1px solid rgba(17,24,39,.06)'}},React.createElement('span',{style:{fontWeight:800}},x),React.createElement('span',{className:'ftv-copilot-pill'}, analyticsBundle?.loadedAt?'branché':'prêt')))
            )
        )
    );
}
...
function App() {
    ...
    const loadYouTubeAnalyticsData = useCallback(async () => {
        if (false && !analyticsAccessToken) { setAnalyticsStatus({ type: 'error', message: 'Connecte YouTube Analytics avant de charger les données.' }); setMainTab('analytics'); return; }
        try {
            setAnalyticsStatus({ type: 'warn', message: 'Chargement des données YouTube Analytics…' });
            const bundle = await fetchAnalyticsBundle({ accessToken: analyticsAccessToken, videos: allVideos, dateRange, channelKey, onProgress: msg => setAnalyticsStatus({ type: 'warn', message: msg }) });
            setAnalyticsBundle(bundle);
            setAnalyticsStatus({ type: bundle.errors?.length ? 'warn' : 'ok', message: `Analytics chargé : ${fmtFull(bundle.summary.views || 0)} vues Analytics · ${fmtFull(Math.round(bundle.summary.watch || 0))} min watch time.` });
        } catch(e) {
            setAnalyticsStatus({ type: 'error', message: e.message || String(e) });
        }
    }, [analyticsAccessToken, allVideos, dateRange, channelKey]);
    ...
    const globalSearchVideos = useMemo(() => {
        ...
        if (!q)
            return [];
        return allVideos.filter(v => videoMatchesSearch(v, q));
    }, [allVideos, deferredSearchQuery]);
    useEffect(() => {
        const q = deferredSearchQuery.trim();
    ...
    }, [dateFilteredVideos, deferredSearchQuery, globalSearchVideos]);
    ...
    const sportsData = useMemo(() => buildSports(filteredVideos), [filteredVideos, overrideNonce]);
    ...
    const handleChannelChange = useCallback((nextKey) => {
        ...
        const run = () => {
            ...
            if (savedKey && savedKey !== apiKey)
                setApiKey(savedKey);
            // D'abord mémoire session, ensuite cache localStorage, sinon vide/démo.
            let state = channelMemoryRef.current[cfg.key];
            if (!state) {
                const channelCache = loadCache(cfg.key);
                const videos = prepareVideosForDashboard(channelCache ? channelCache.videos : (cfg.key === 'sport' ? reclassifiedMocks : []), 0);
                state = { videos, isDemo: !channelCache && cfg.key === 'sport', cacheTs: channelCache?.ts || null };
                channelMemoryRef.current[cfg.key] = state;
            }
            const nextVideos = state.videos || [];
            setChannelKey(cfg.key);
            setAllVideos(nextVideos);
            setIsDemo(!!state.isDemo);
            setCacheTs(state.cacheTs || null);
            setError('');
    ...
    const handleLoad = useCallback(async () => {
        ...
        try {
            ...
            try {
                setLoadingMsg('Lecture du snapshot serveur déjà préparé…');
                const snapshot = await window.fetchChannelSnapshot(channelKey);
                snapshotMeta = snapshot;
                videos = prepareVideosForDashboard(snapshot.videos || []);
            } catch (snapshotError) {
                setLoadingMsg('Snapshot indisponible : chargement API direct de secours…');
                const fetchStart = window.getDefaultFetchStartDate ? window.getDefaultFetchStartDate(channelKey) : undefined;
                videos = prepareVideosForDashboard(await fetchAll(key, msg => setLoadingMsg(msg), fetchStart, channelKey));
            }

            const now = snapshotMeta?.generatedAt ? new Date(snapshotMeta.generatedAt).getTime() : Date.now();
            const cacheSaved = saveCache(videos, channelKey);
            if (!cacheSaved)
                setError('Données chargées, mais le cache local du navigateur est plein : vide le cache de cette chaîne si besoin.');
            ...
            // Hybride live : on affiche instantanément le snapshot, puis on actualise en arrière-plan
            // uniquement les vues/likes/commentaires publics. Analytics reste en snapshot.
            window.refreshPublicStatsLive(videos, msg => setLoadingMsg(msg))
              .then(liveVideos => {
                const prepared = prepareVideosForDashboard(liveVideos);
                const liveTs = Date.now();
                channelMemoryRef.current[channelKey] = { videos: prepared, isDemo: false, cacheTs: liveTs };
                setAllVideos(prepared);
                setCacheTs(liveTs);
                saveCache(prepared, channelKey);
              })
              .catch(() => {})
    ...
    return (React.createElement("div", { style: { display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg)' } },
        ...
        React.createElement("div", { style: { width: 244, flexShrink: 0, background: 'var(--bg2)', borderRight: '1px solid var(--border)',
                ...
                React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 } },
                    ...
                    React.createElement("div", null,
                        React.createElement("select", { value: channelKey, onChange: e => handleChannelChange(e.target.value), title: "Changer de cha\u00EEne YouTube", style: {
                            ...
                            } }, Object.values(CHANNEL_CONFIGS).map(ch => (React.createElement("option", { key: ch.key, value: ch.key }, ch.label)))),
            ...
            React.createElement("div", { style: { padding: '9px 9px', borderTop: '1px solid var(--border)' } },
                !showApiInput ? (React.createElement("div", null,
                    React.createElement("button", { onClick: () => { if (!loading && (isDemo || !allVideos.length)) handleLoad(); }, style: {
                        ...
                        React.createElement("span", { style: { fontSize: 11, color: 'var(--text2)' } }, isDemo ? 'Données démo' : (allVideos.length ? 'Snapshot + live' : 'Aucune donnée')),
                        React.createElement("span", { style: { fontSize: 10, color: 'var(--text3)', marginLeft: 'auto' } }, loading ? 'Chargement…' : (isDemo || !allVideos.length ? 'Auto-load' : 'Live léger'))),
                    ...
                    !isDemo && allVideos.length > 0 && React.createElement("button", { onClick: () => {
                            if (confirm('Vider le cache de cette chaîne et recharger les vidéos depuis YouTube ?\nCela force une re-classification complète avec les règles courantes.')) {
        ...
        React.createElement("div", { style: { flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' } },
            React.createElement("div", { style: { padding: '0 18px', background: 'var(--bg2)', borderBottom: '1px solid var(--border)',
                    display: 'flex', gap: 0, flexShrink: 0, alignItems: 'stretch' } },
                [['chiffres', '📊 Chiffres'], ['graphiques', '📈 Graphiques'], ['previsions', '🔭 Prévisions'], ['copilot', '🧠 Copilot IA'], ['analytics', '📡 Analytics']].map(([id, label]) => (React.createElement("button", { key: id, onClick: () => setMainTab(id), style: {
                ...
                React.createElement("div", { style: { marginLeft: 'auto', display: 'flex', alignItems: 'center' } },
                    React.createElement("button", { onClick: () => setUserMenuOpen(v => !v), "aria-label": userMenuOpen ? 'Fermer le menu profil' : 'Ouvrir le menu profil', style: {
                            width: 34, height: 34, borderRadius: 12, border: '1px solid var(--border2)', background: userMenuOpen ? 'var(--red)' : 'var(--bg4)',
                            color: userMenuOpen ? '#fff' : 'var(--text)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3,
                            cursor: 'pointer', transition: 'all .15s', boxShadow: userMenuOpen ? '0 10px 24px rgba(226,0,26,.18)' : 'none', position: 'relative', zIndex: userMenuOpen ? 902 : 1
                ...
                userMenuOpen && React.createElement(React.Fragment, null,
                    React.createElement("div", { onClick: () => setUserMenuOpen(false), style: { position: 'fixed', inset: 0, zIndex: 900, background: 'rgba(15,23,42,.10)', backdropFilter: 'blur(1px)' } }),
                    React.createElement("aside", { onClick: e => e.stopPropagation(), style: { position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(360px,92vw)', zIndex: 901,
                            background: 'var(--bg2)', borderLeft: '1px solid var(--border)', boxShadow: '-24px 0 70px rgba(15,23,42,.18)',
                            padding: 22, boxSizing: 'border-box', animation: 'ftvSlideMenuIn .22s ease-out', display: 'flex', flexDirection: 'column', gap: 18 } },
                            ...
                            React.createElement("div", { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 } },
                                ...
                                React.createElement("div", { style: { gridColumn: '1 / -1', border: '1px solid var(--border2)', borderRadius: 14, padding: 10, background: 'var(--bg2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 } },
                                    React.createElement("div", null,
                                        ...
                                        React.createElement("div", { style: { fontSize: 13, fontWeight: 800, color: 'var(--text)', marginTop: 3 } }, "Plateforme locale")),
                        ...
                        React.createElement("div", { style: { marginTop: 'auto', display: 'grid', gap: 8 } },
                            React.createElement("button", { onClick: () => { setUserMenuOpen(false); window.ftvAuthLogout && window.ftvAuthLogout(); }, style: { width: '100%', border: 'none', borderRadius: 14, padding: '12px 14px', background: 'var(--red)', color: '#fff', fontWeight: 900, fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' } }, "D\u00E9connexion"),
                            React.createElement("div", { style: { fontSize: 10, color: 'var(--text3)', lineHeight: 1.45, textAlign: 'center' } }, "Donn\u00E9es de profil stock\u00E9es localement dans ton navigateur. En production, elles seront reli\u00E9es \u00E0 une base s\u00E9curis\u00E9e."))))),
            React.createElement("div", { style: { padding: '12px 18px', borderBottom: '1px solid var(--border)', display: 'flex',
                ...
                React.createElement(KpiCard, { label: "Vid\u00E9os", value: nV, format: n => n.toLocaleString('fr-FR'), accent: TYPE_COLORS.video, delay: 50 }),
                ...
                React.createElement("div", { style: { display: 'flex', gap: 7, flexShrink: 0, marginLeft: 'auto', alignItems: 'center', position: 'relative' } },
                    React.createElement("button", { onClick: () => setShowCal(c => !c), style: {
                        ...
                        periodLabel),
                    React.createElement("button", { onClick: () => { setExportMode(m => { if (m)
                    ...
                    showCal && (React.createElement("div", { style: { position: 'absolute', top: '100%', right: 0, marginTop: 6, background: '#fff',
                            borderRadius: 16, boxShadow: '0 12px 40px rgba(0,0,0,0.18)', padding: 20, width: 315, zIndex: 300 }, onClick: e => e.stopPropagation() },
            ...
            React.createElement("div", { style: { padding: '10px 18px', display: 'flex', gap: 7, flexShrink: 0,
...
function CalendarPickerInline({ range, onChange, extraRanges = [], onExtraChange, onClose }) {
    ...
    return (React.createElement("div", null,
        React.createElement("div", { style: { display: 'flex', gap: 3, marginBottom: 11, padding: 3, background: 'var(--bg4)', borderRadius: 8 } }, [['day', 'Jour'], ['week', 'Semaine'], ['month', 'Mois'], ['year', 'Année']].map(([k, lbl]) => (React.createElement("button", { key: k, onClick: () => setMode(k), style: {
                flex: 1, fontSize: 11, padding: '5px 0', border: 'none', cursor: 'pointer', fontFamily: 'var(--font)',
                borderRadius: 6, fontWeight: mode === k ? 600 : 400,
        ...
        (mode === 'day' || mode === 'week') && (React.createElement(React.Fragment, null,
            ...
            React.createElement("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2, marginBottom: 4 } }, DAYS_FR.map((d, i) => React.createElement("div", { key: i, style: { textAlign: 'center', fontSize: 9.5, fontWeight: 600,
                    color: 'var(--text3)', padding: '2px 0' } }, d))),
            React.createElement("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2 } }, cells.map((d, i) => {
                if (!d)
                ...
                return (React.createElement("button", { key: i, onClick: (ev) => handleDay(d, ev), style: {
                        ...
                        border: 'none', borderRadius: 6, padding: '5px 2px', fontSize: 11.5,
                        fontWeight: fw, cursor: 'pointer', transition: 'background .1s', fontFamily: 'var(--font)'
                    } }, d.getDate()));
            })))),
